...
start-hytale-server:
    cd hytale-server && /usr/lib/jvm/java-25-openjdk/bin/java -jar Server/HytaleServer.jar -assets Assets.zip --disable-sentry; cd -

update-mod:
    #!/usr/bin/env sh
    rm target/HiddenTeleporters-*.jar || true
    JAVA_HOME=/usr/lib/jvm/java-25-openjdk mvn package
    mkdir -p hytale-server/mods
    rm hytale-server/mods/HiddenTeleporters-*.jar || true
